CLASS lcl_buffer DEFINITION.
  PUBLIC SECTION.
    ...
    TYPES: BEGIN OF ty_buffer_master.
             INCLUDE TYPE zhc_master_img AS data.
...
CLASS lhc_HCMMaster DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    methods:
      ...
      read   for read   importing keys     for read   hcmmaster
                        result    result.
...
*-- Nombre técnico usuario actual
    DATA(lv_uname) = cl_abap_context_info=>get_user_technical_name( ).

*-- Get último número de empleado
    SELECT MAX( e_number ) FROM zhc_master_img
  ...
  METHOD update.
    ...
    LOOP AT entities INTO DATA(ls_entities).
      ...
      SELECT SINGLE * FROM zhc_master_img
      ...
      data = VALUE #( e_number = ls_entities-%data-e_number
                     ...
                     m_name       = COND #( WHEN ls_entities-%control-m_name EQ if_abap_behv=>mk-on
                                            ...
                                            ELSE ls_ddbb-m_name )
                     m_Departament = COND #( WHEN ls_entities-%control-E_Department EQ if_abap_behv=>mk-on
                                             THEN ls_entities-%data-E_Department
                                             ELSE ls_ddbb-e_department )

                    crea_date_time = ls_ddbb-crea_date_time crea_uname = ls_ddbb-crea_uname ) ) INTO TABLE lcl_buffer=>mt_buffer_master.

      IF NOT ls_entities-e_number IS INITIAL.

        INSERT VALUE #( %cid = ls_entities-%data-e_number e_number = ls_entities-%data-e_number ) INTO TABLE mapped-hcmmaster.
...
ENDCLASS.
CLASS lsc_Z_I_HCM_MASTER_IMG DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_Z_I_HCM_MASTER_IMG IMPLEMENTATION.
  ...
  METHOD save.
    DATA: lt_data_created TYPE STANDARD TABLE OF zhc_master_img,
          lt_data_updated TYPE STANDARD TABLE OF zhc_master_img,
          lt_data_deleted TYPE STANDARD TABLE OF zhc_master_img.
    ...
    IF NOT lt_data_created IS INITIAL.
      INSERT zhc_master_img FROM TABLE @lt_data_created.
    ENDIF.
    ...
    IF NOT lt_data_updated IS INITIAL.
      UPDATE zhc_master_img FROM TABLE @lt_data_updated.
    ENDIF.
    ...
    IF NOT lt_data_deleted IS INITIAL.
      DELETE zhc_master_img FROM TABLE @lt_data_deleted.
    ENDIF.
